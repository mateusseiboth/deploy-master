...
import type { IContainerOrchestrator, IDnsProvider } from "@modules/deploy/domain/ports";
...
export class RegisterDnsStep extends DeployStep {
  ...
  async execute(ctx: DeployContext): Promise<void> {
    ...
    ctx.log(`Registrando DNS ${ctx.hostname} -> ${env.dns.reverseProxyIp}`);
    await this.dns.register(ctx.hostname, env.dns.reverseProxyIp);

    // Valida propagação (best-effort: o HealthCheck é o gate final para READY).
    const propagated = await this.dns.waitForPropagation(ctx.hostname, env.dns.reverseProxyIp);
  ...
  override async compensate(ctx: DeployContext): Promise<void> {
    if (ctx.hostname) await this.dns.unregister(ctx.hostname);
  }
}
...
export class HealthCheckStep extends DeployStep {
  ...
  private async runChecks(ctx: DeployContext): Promise<Record<string, boolean>> {
    const [dns, container, proxy, database] = await Promise.all([
      this.dns.isResolving(ctx.hostname!, env.dns.reverseProxyIp),
      this.docker.isHealthy(ctx.containerId!),
      tcpProbe(env.dns.reverseProxyIp, 443),
